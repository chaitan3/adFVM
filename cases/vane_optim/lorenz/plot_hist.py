...
#for beta in [0.0]:
    ...
    f = 'beta_{}/optim_nd2_rastigrin8.pkl'.format(beta)
    #f = 'beta_{}/optim_nd2_rosenbrock5.pkl'.format(beta)
    try:
        a = pkl.load(open(f))
    except:
        continue
    ...
    #plt.plot(y.mean(axis=0), label=r'$\beta$ = {}'.format(beta))
    #plt.plot(np.median(y, axis=0), label=r'$\beta$ = {}'.format(beta))
    plt.semilogy(np.median(y, axis=0), label=r'$\beta$ = {}'.format(beta))
