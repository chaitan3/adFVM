...
def adjoint(stackedAdjointFields):
    stackedAdjointFields = parallel.scatterCells(stackedAdjointFields, meshO)
    stackedAdjointFields[:meshO.nInternalCells] *= meshO.volumes
    stackedAdjointFields[meshO.nInternalCells:] *= meshO.areas[meshO.nInternalFaces:]
    gradient = primal.gradient(stackedFields, stackedAdjointFields, dt)[0]
    gradient[:meshO.nInternalCells] /= meshO.volumes
    gradient[meshO.nInternalCells:] /= meshO.areas[meshO.nInternalFaces:]
    gradient = parallel.gatherCells(gradient, meshO)
    return gradient
...
for sim in range(0, nEigen):
    stackedAdjointFields = parallel.scatterCells(vectors[sim], meshO)
